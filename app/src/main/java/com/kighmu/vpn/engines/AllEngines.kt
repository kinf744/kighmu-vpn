            ...
            for (port in ports) {
                ...
                if (v2rayPoint == null && vpnService != null) {
                    try {
                        ...
                        val dialer = chzPsiphonAndV2ray.KighmuV2RayCallback(vpnService)
        ...
        val config = """
{
  ...
  "insecure": true,
  "socks5": {
    "listen": "127.0.0.1:${hConfig.socksPort.takeIf { it > 0 } ?: run { _socksPort = Companion.getFreePort(); _socksPort }}"
  }
}
"""
    ...
    private fun startFdControlServer(sockPath: String, vpnSvc: android.net.VpnService?) {
        ...
        // Forcer la suppression du fichier socket
        try { fdControlServerSocket?.close() } catch (_: Exception) {}
        fdControlServerSocket = null
        val sockFile = java.io.File(sockPath)
        sockFile.delete()
        // Attendre que le fichier soit supprimé
        Thread.sleep(100)
        val serverSocket = try {
            android.net.LocalServerSocket(sockPath)
        } catch (e: Exception) {
            logHysteria("fdControl socket error: ${e.message}")
            return
        }
                ...
                while (running) {
                    ...
                    Thread {
                        try {
                            ...
                            fds?.forEach { fd ->
                                try {
                                    val fdInt = android.system.Os.fcntlInt(
                                        fd, android.system.OsConstants.F_DUPFD_CLOEXEC, 3)
                                    val result = vpnSvc.protect(fdInt)
                                    logHysteria("fdControl protect=$result")
                                    try { android.system.Os.close(fd) } catch (_: Exception) {}
                                } catch (e: Exception) {
                                    logHysteria("fdControl fd error: ${e.message}")
                                }
                            }
                        } catch (e: Exception) {
