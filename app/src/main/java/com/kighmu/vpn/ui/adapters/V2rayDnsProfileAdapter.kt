...
class V2rayDnsProfileAdapter(
    private val profiles: MutableList<V2rayDnsProfile>,
    private val onSelectionChanged: (String, Boolean) -> Unit,
    private val onEdit: (V2rayDnsProfile) -> Unit,
    private val onDelete: (V2rayDnsProfile) -> Unit,
    private val onClone: (V2rayDnsProfile) -> Unit
) : RecyclerView.Adapter<V2rayDnsProfileAdapter.VH>() {
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        holder.tvName.text = if (p.profileName.isNotEmpty()) p.profileName else "V2ray+DNS ${position + 1}"
        holder.tvSub.text = "${p.protocol.uppercase()} → ${p.serverAddress}:${p.serverPort} (${p.transport}) | DNS: ${p.dnsServer}"
    ...
    fun setProfiles(list: List<V2rayDnsProfile>) {
        profiles.clear()
